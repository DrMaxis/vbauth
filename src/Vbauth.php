 ...
 *
 * Original Author: Przemyslaw Peron (https://github.com/przemyslawperon)
 * Modified for Laravel 5 By: Posttwo (posttwo@posttwo.pt)
 *
 */
...
class Vbauth
{
    ...
    public function __construct()
    {
		...
		$this->permission  	  = Config::get('vbauth.permissions');
    ...
    public function is($group)
    {
	 ...
	 /**
     * Checks to see if the current user has permission to perform an action
     * Permission mapping is in config file.
     *
     * @param	string		Permission varname
     * @return	boolean		True = has permission; false does not have permission
     */
	 
	public function can($permission)
	{
		 if (empty($this->permission[$permission])) {
            return false;
        }
        ...
        return (bool) count(array_intersect($my_groups, $this->permission[$permission]));
	}
